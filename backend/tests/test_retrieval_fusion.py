...
def test_rrf_overlap_boosts_shared_item():
    # "b" appears rank-2 in list1 and rank-1 in list2 — should outscore "a" (only list1 rank-1)
    result = rrf_fuse(["a", "b"], ["b", "c"])
    scores = {r[0]: r[1] for r in result}
    assert scores["b"] > scores["a"]
    assert scores["b"] > scores["c"]
    # Exact check: b = 1/(60+2) + 1/(60+1), a = 1/(60+1)
...
def test_rrf_zero_args_returns_empty():
    assert rrf_fuse() == []
